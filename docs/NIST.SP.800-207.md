...
posture.
## Keywords
architecture; c ybersecurity; e nterprise; network security; zero trust.
...
## 2
Any mention of commercial products or services within NIST documents is for information only; it does not imply a
...
## 3
Part of the concepts defined in OASIS XACML 2.0 https://docs.oasis-open.org/xacml/2.0/access_control-xacml-2.0-core-spec-
os.pdf
...
basic tenets:
- All data sources and computing services are considered resources. A network may be
...
resources.
- All communication is secured regardless of network location. Network location alone
...
- Access to resources is determined by dynamic policy—including the observable state
...
- The entire enterprise private network is not considered an implicit trust zone. Assets
...
- Devices on the network may not be owned or configurable by the enterprise. Visitors
...
resources.
- No resource is inherently trusted.  Every asset must have its security posture evaluated
...
enterprise resource.
- Not all enterprise resources are on enterprise-owned infrastructure. Resources
...
above.
- Assets and workflows moving between enterprise and nonenterprise infrastructure
...
- Threat intelligence feed(s): T his provides information from internal or external sources
...
whereas a singular TA may fail to detect the new behavior. In another example,  an a ccountant
...
- Enterprise assets have basic network connectivity. The local area network (LAN),
...
- The enterprise must be able to distinguish between what assets are owned or
...
- The enterprise can observe all network traffic. The enterprise records packets seen on
...
- Enterprise resources should not be reachable without accessing a PEP. Enterprise
...
- The data plane and control plane are logically separate. The policy engine, policy
...
- Enterprise assets can reach the PEP component. Enterprise subjects must be able to
...
connection.
- The PEP is the only component that accesses the policy administrator as part of a
...
- The infrastructure used to support the ZTA access decision process should be made
...
- Enterprise assets may not be able to reach certain PEPs due to policy or observable
